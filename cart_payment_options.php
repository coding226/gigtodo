        ...
        <?php if($current_balance >= $sub_total ){ ?>
		   ...
		   <div class="col-md-12 mb-3">
					<div class="card payment-options">
						...
						<div class="card-body">
							<div class="row">
								...
								<div class="col-12">
									<p class="lead mt-2">
									<a id="shopping-balance" href="<?= $exchange_url; ?>" target="_blank"class="btn btn-lg btn-success btn-block" >Buy VFY Token</a>

									</p>
									<p class="lead mt-2">
									<button id="shopping-balance"  class="btn btn-lg btn-success btn-block enableEthereumButton" >Pay using VFY token</button>
									</p>
								</div>
							</div>
						</div>
				...
				<div class="col-md-12 mb-3">
					<div class="card payment-options">
						<div class="card-header">
							<h5><i class="fa fa-credit-card"></i> Other Payment Options</h5>
						</div>
						<div class="card-body">
...
var abi = JSON.parse(('<?php echo html_entity_decode($contract_abi); ?>'.trim() || '[]'));
var bytecode = '<?php echo $contract_bytecode; ?>';

var contract = new ethers.Contract('<?php echo $contract_address; ?>', abi);
async function getAccount() {
  accounts = await ethereum.request({ method: 'eth_requestAccounts' });
 let calculated_amount = ethers.BigNumber
  .from(parseFloat('<?php echo $total;?>'))
  .mul(parseInt('<?php echo $token_value;?>' || 1 ))
  .mul(ethers.BigNumber.from(10).pow(parseInt('<?php echo $contract_decimals;?>')))
  
  .toString()
  var res = await contract.populateTransaction.transfer('<?php echo $wallet_address; ?>', calculated_amount)
  ethereum
    .request({
      ...
      params: [
        {
          from: accounts[0],
          to: '<?php echo $contract_address; ?>',
		  value: '0x0',
		  data: res.data
        },
      ],
    })
    .then((txHash) => window.location.href = "<?= $site_url; ?>/token_payment")
    .catch((error) => {
		alert_error('Payment failure. Please try again.');
	});
}
</script>
</body>
